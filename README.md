...
9. CPU 성능 향상 기법성능 지표: 클락(Clock), 코어(Core), 스레드(Thread)가 중요합니다. 하드웨어 스레드는 하나의 코어가 동시에 처리하는 명령어 단위로 '논리 프로세서'라고도 합니다.
    명령어 파이프라인: 인출 → 해석 → 실행 → 결과 저장의 단계를 겹쳐서 처리하는 방식입니다.
   파이프라인 위험: 데이터 의존성(데이터 위험), 제어 위험, 구조적 위험으로 인해 성능 향상에 실패할 수 있습니다.
   CISC vs RISC:
   CISC: 복잡하고 다양한 명령어를 사용하며 파이프라이닝에 불리합니다 (예: x86-64).
   RISC: 적고 규격화된 명령어를 사용하여 효율적입니다.

11. 메모리와 저장 장치
    RAM의 종류: DRAM(동적), SRAM(정적), SDRAM(동기식), DDR SDRAM 등이 있습니다.
    주소 체계: 하드웨어 상의 물리 주소와 프로그램에 부여된 논리 주소가 있으며, 베이스/한계 레지스터를 통해 관리합니다.
    캐시 메모리: SRAM 기반으로 CPU와 메모리 사이의 속도 차이를 줄입니다.참조 지역성의 원리(시간 지역성, 공간 지역성)를 활용합니다.
    RAID: 여러 개의 물리적 보조기억장치를 하나의 논리적 장치처럼 사용하는 기술로, 데이터를 분산 저장하는 스트라이핑(Striping) 방식 등이 있습니다.

12. 입출력 방식 및 운영체제 기초
    입출력 방식:프로그램 입출력: 명령어로 직접 제어합니다.
    메모리 맵 입출력: 메모리 주소와 장치 주소를 하나의 공간으로 간주합니다.
    고립형 입출력: 메모리와 장치 주소 공간을 분리합니다.
    인터럽트/DMA 기반: 인터럽트나 직접 메모리 접근(DMA)을 통해 제어합니다.

운영체제(OS): 시스템 자원을 관리하며, 사용자/커널 모드의 이중 모드와 시스템 호출을 통해 작동합니다. 프로세스 관리를 위해 **PCB(Process Control Block)**를 사용하며, 프로세스 간 전환 시 Context Switching이 발생합니다.
